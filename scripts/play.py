...
parser = argparse.ArgumentParser(description="Evaluate Go2 Ball Kick Policy")
parser.add_argument("--num_envs", type=int, default=16, help="Number of robots to render.")
...
# 3. Environment & Scene Definition
@configclass
...
class Go2BallKickEnvCfg(ManagerBasedRLEnvCfg):
    scene: Go2BallKickSceneCfg = Go2BallKickSceneCfg(num_envs=16, env_spacing=3.0)
...
def main():
    ...
    checkpoint_path = os.path.join(log_dir, "model_1500.pt")
    if not os.path.exists(checkpoint_path):
        import glob
        checkpoints = sorted(glob.glob(os.path.join(log_dir, "model_*.pt")))
        checkpoint_path = checkpoints[-1] if checkpoints else None

    print(f"[INFO] Loading policy checkpoint from: {checkpoint_path}")
    runner.load(checkpoint_path)
    policy = runner.get_inference_policy(device="cuda:0")

    obs, _ = vec_env.reset()
    print("[INFO] Rendering 3D evaluation loop. Press Ctrl+C in terminal to exit.")
